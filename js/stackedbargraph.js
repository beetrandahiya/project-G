...
function newStackedBarGraph(containerDOM, graphData, layout) {
    ...
    for (dataindex = 0; dataindex < graphData.length; dataindex++) {
        ...
        var stack_style = data.stack.stackstyle||"horizontal";
        ...
        for (i = 0; i <= data_y.length - 1; i++) {
            ...
            w_stackpart = wp/data_y[i].length;


            j=0;
            xval= padding+w0*i+(w0/2)-(wp/2);
            while(j<=data_y[i].length-1){

                console.log("done");

                yval = ((data_y[i][j] - y_min) * (h_graph) / (y_max - y_min));
                ...
                bar.setAttribute("width",w_stackpart);
